...
function checkEnding(route){
  ...
  setTimeout(()=>{
    let ending;
    if(route==='haidi'){
      if(State.haidi >= AFFINITY_MAX) ending = 'ending_haidi_good';
      else if(State.haidi * 10 > AFFINITY_NORMAL_PCT) ending = 'ending_haidi_normal';
      else ending = 'ending_bad';
    } else if(route==='duorou'){
      if(State.duorou >= AFFINITY_MAX) ending = 'ending_duorou_good';
      ...
      if(State.shalaxi >= AFFINITY_MAX) ending = 'ending_slaneesh';
      else ending = 'ending_bad';
    } else if(route==='ss'){
      if(State.ss >= AFFINITY_MAX) ending = 'ending_ss_good';
      else if(State.ss * 10 > AFFINITY_NORMAL_PCT) ending = 'ending_ss_normal';
      else ending = 'ending_bad';
    } else {
      ending = 'ending_bad';
    }
...
/* Chapter 2: 远处似乎有些热闹 —— 跳进 re_* 小剧场，看完回 p2_9_1 */
function pickDailyRandomEvent(){
  const scenes = {
    zhou:['re_zhou1','re_zhou2'],
    huyou:['re_huyou1','re_huyou2'],
    tan:['re_tan1','re_tan2'],
  ...
  for(const key of Object.keys(scenes)){
    if(!State.randomEncountered[key]) pool.push(key);
  }
  // 都看过则允许再随机抽一位（仍能进台词）
  const keys = pool.length > 0 ? pool : Object.keys(scenes);
...
function applyDailyEventFallbackReward(){
...
function startDailyEvent(){
  if(State.eventToday || State.energy < 1) return;
  State.energy = Math.max(0, State.energy - 1);
  showAttributePopup('energy', -1);
  State.eventToday = true;
  updateHUD();

  const reId = pickDailyRandomEvent();
  if(reId && Scenes[reId]){
    State.randomReturnTo = 'p2_9_1';
    $('fade-overlay').classList.add('show');
    setTimeout(()=>{
      renderScene(reId);
      $('fade-overlay').classList.remove('show');
  ...
  // 兜底：资源事件短剧情
  applyDailyEventFallbackReward();
  setTimeout(()=>renderScene('p2_9_event'), 400);
}

/* Chapter 2 onEnter: 每日随机事件（旧入口兼容，已改由 startDailyEvent） */
...
// 预览结局：?ending=ligong_wtc / haidi_good / duorou_normal / bad 等
(function previewEndingFromQuery(){
  const q = new URLSearchParams(location.search).get('ending');
  if(!q) return;
  const map = {
    ligong_wtc: 'ending_ligong_wtc',
    ligong_good: 'ending_ligong_good',
    ligong_normal: 'ending_ligong_normal',
    haidi_good: 'ending_haidi_good',
    haidi_normal: 'ending_haidi_normal',
    duorou_good: 'ending_duorou_good',
    duorou_normal: 'ending_duorou_normal',
    ss_good: 'ending_ss_good',
    ss_normal: 'ending_ss_normal',
    slaneesh: 'ending_slaneesh',
    bad: 'ending_bad'
  };
  const id = map[q] || (q.startsWith('ending_') ? q : 'ending_' + q);
  if(!Scenes[id]) return;
  $('title-screen').classList.add('hidden');
  State.__endingLock = true;
  setTimeout(()=>renderScene(id), 100);
})();
